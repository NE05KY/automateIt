...
4. ...Графиков
5. ...Карты 
6. ...Векторных иллюстраций

<footer>
Несмотря на все преимущества SVG-графики, это не лучший выбор для любой ситуации.
SVG покрывает свой строго определенный спектр задач.
В первую очередь это элементы пользовательского интерфейса.
Логотипы как текстовые так и с изображениями.
Иконки являются самой популярной сферой применения SVG.
Я думаю многие работали с d3.js - популярной библиотекой для визуализации данных.
А так же для векторных илюстраций, которые стали популярны после того, 
как флэт-дизайн захватил веб.
</footer>

## **Изображения**

<footer>
Для начала поговорим о изображениях.
</footer>
...
<footer>
В векторе можно отрисовывать целые картины, однако пропорционально количеству деталей
растёт и размер файла. И начиная с определенного момента, теряется преимущество в размере файла над растровой графикой.
Однако по прежнему сохраняется масштабируемость до любых разрешений.
Я думаю всем понятно, что SVG абсолютно не подходит для фотореалистичной графики.
</footer>
...
<footer>
Однако если деталей не так много, SVG сохраняет все свои преимушества,
и векторный йода весит почти в 4 раза меньше при том же количестве силы.
</footer>
...
<footer>
Существует целых 6 способов внедрения SVG-изображений на страницу,
начинаю от классического img-тега, и заканчивая инлайном svg.
В рамках своего доклада я не буду подробно останавливаться на особенностях каждого способо,
отмечу только что последний вариант даёт больше всего преимуществ.
</footer>

## **Иконки**  

<footer>
Следующий вариант использования SVG - иконки.
Это то, для чего SVG подходит просто идеально. 
Небольшие изображение с маленьким количеством деталей в векторе будут весить несколько раз меньше 
и выглядеть заметно четче.
...
Обычно их количество плавно стремится к сотне.
И даже на обычном лендинге их встречается несколько десятков.
Но чем больше становится иконок, тем сложнее этой массой управлять. 
...
<footer>
Для вставки на страницу обычно используется предназначенный совсем не для этого html-тег i.
Изначально этот тег устанавливал курсивное начертание шрифта, италик. 
Но так как для иконок не было отдельного элемента, начали использовать его.
Как вы видите, вставить иконку из шрифта можно двумя способами:
использовать css-class или спецсимвол из юникода.
Более популярна первая техника, так как она более семантичная.
</footer>
...
например таким, как  Font Awesome. Почти 600 иконок на все случаи жизни.
</footer>
...
Или если вы следите за перфомансом и вам в проекте не нужно 600 иконок?
...
<footer>
Окей, с использованием разобрались, давайте поговорим о том, как же рисовать векторную графику
</footer>
...
<footer>
Существует большое количество редакторов на любой вкус, под все платформы и с любой стоимостью (от 0 до много).
Но так же, при наличии необходимого опыта, SVG можно писать в любом текстовом редакторе.
Абсолютно любом. Да, даже в блокноте, для самых смелых.
Но на самом деле существуют плагины под среды разработки, которые рядом с исходным кодом svg
показывают превью изображения, что сильно упрощает процесс рисования буквами.
</footer>
...
<footer>
Если SVG можно редактировать в текстовом редакторе, то что же такое у него внутри?
Обычная XML-разметка, приправленная стилями для примитивов. 
Причём стили могут задаваться как обычным css-кодом, так и через атрибуты.
</footer>
...
## Что внутри?
{:.cover #CrocoDevTools}
![](pictures/crocodile-devtools.png)  
<footer>
И самое крутое во всё этом то, что в браузере все примитивы являются обычными DOM-нодами. 
Это позволяет взаимодействовать с любым элементом с помощью JS.
</footer>
...
<footer>
Всё это позволяет делать примерно вот такие интерактивные анимации.
На двух предыдущих слайдах был код этого крокодила.
Это обычная заинлайненная svg картинка, на элементы которой навешены JS слушатели.
И тем же JS контролируется вся анимация.
</footer>
...
<footer>
Мой доклад был сильно ограничен форматом блица, и я не успел вам о многом рассказать.
Если вы бедете работать с вектором, обязательно пользуйтесь SVGO, 
утилитой которая может до 70% сократить объем файла, а счет удаления мета-информации и прочего.
Так как мы можем стилизовать простым css, то нам доступны все преимущества media queries, 
что позволяет делать responsive изображения (увеличивать количество деталей с ростом разрешения)
Я не затронул тему анимации и интерактивности, но в этом поле всё сильно упрощают js-библиотеки,
такие как snap.svg, на которой и был создан предыдущий пример.
Ну и внутреннее устройство svg - это целый дивный мир, с огромным количеством возможностей.
...
<footer>
На этом у меня всё, если у вас есть вопросы по моему доклады, на часть из них я успею ответить сейчас,
а остальные вы можете задать мне лично в кулуарах. Спасибо за внимание.
</footer>